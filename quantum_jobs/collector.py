...
from quantum_jobs.sources.lever_companies import (
...
DB_PATH = REPO_ROOT / "quantum_jobs.db"
TIMEOUT_S = 30


logging.basicConfig(
    filename=str(REPO_ROOT / "collector.log"),
    level=logging.INFO,
...
# Backward-compat export for tests/utilities that referenced collector.normalize_str.
normalize_str = db.normalize_str


class TeamMeSource:
    """
    TeamMe-like endpoint adapter (Quantum Machines).
    """
    ...
    def fetch_rows(self, pulled_at: str) -> List[Dict[str, Any]]:
        ...
        for job in jobs:
            if not isinstance(job, dict):
                continue
            rows.append(db.make_row_from_normalized(
                pulled_at=pulled_at,
...
def main() -> None:
    ...
    try:
        ...
        for src in sources:
            ...
            prev_time = db.get_previous_snapshot_time(conn, company, pulled_at)
            new_map = {r["job_id"]: r for r in rows}
            prev_map = db.load_snapshot_map(conn, company, prev_time) if prev_time else {}
            db.log_changes(conn, pulled_at=pulled_at, company=company, prev_map=prev_map, new_map=new_map)
            db.upsert_current(conn, rows)
